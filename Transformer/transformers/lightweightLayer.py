...
from utillayer import Norm, MultiHeadAttention, FeedForward
...
class LightweightConvLayer(nn.Module):
    ...
    def forward(self, x, mask):
        x = self.linear_1(x)
        x = self.activation(x)
        conv_mask = mask[:, -1, :]  # batch_size, seq_len
        conv_mask = conv_mask.unsqueeze(-1)  # batch_size, seq_len, d_model
